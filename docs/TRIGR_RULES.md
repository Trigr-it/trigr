...
- **Auto-updater URL uses `Trigr-Setup.exe` (no version)** — the download URL and `artifactName` were changed in v0.1.44 to use a consistent filename. Never add a version string back to the artifact name or URL. Users on pre-v0.1.44 builds will fail one auto-update (old versioned filename no longer exists) and must download manually once.
