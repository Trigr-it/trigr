#!/usr/bin/env node
/**
 * Generates public/tray-icon.ico — a 256×256 ICO (PNG-embedded) with the
 * Trigr logo (amber accent on transparent background).
 *
 * Run once:  node scripts/gen-tray-icon.js
 * Also runs automatically as part of:  npm run package
 *
 * Note: The runtime system-tray icon is loaded from app-icon.png by main.js.
 * This ICO is used by electron-builder as the Windows app/installer icon,
 * which requires a minimum of 256×256.
 */
'use strict';
...
// --- Generate 256×256 PNG then wrap in ICO -------------------------------
const SIZE = 256;
const pngData = buildPNG(SIZE, SIZE, (x, y) => makePixel(x, y, SIZE));

// ICO format with embedded PNG (standard for 256×256).
// Width/height bytes are 0 to indicate 256 (max uint8 is 255).
...
const outPath = path.join(__dirname, '..', 'public', 'tray-icon.ico');
fs.writeFileSync(outPath, ico);
console.log(`✓ tray-icon.ico written (${ico.length} bytes, 256×256 PNG-embedded) → ${outPath}`);
