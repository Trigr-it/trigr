...
export default function TitleBar({
  ...
  function handleDragOver(e, idx) {
    ...
    if (idx !== dragIndex && idx !== 0) setDragOverIndex(idx);
  }

  function handleDrop(e, idx) {
    e.preventDefault();
    if (dragIndex !== null && dragIndex !== idx && dragIndex !== 0 && idx !== 0) {
      const newProfiles = [...profiles];
      const [moved] = newProfiles.splice(dragIndex, 1);
      newProfiles.splice(idx, 0, moved);
      onReorderProfiles?.(newProfiles);
    }
    setDragIndex(null);
    ...
    <div className="titlebar" data-drag="true">
      <div className="titlebar-left">
        ...
        {activeArea === 'mapping' && (
          <>
            <div className="titlebar-divider" />
            <div className="profile-tabs" data-drag="false">
          {profiles.map((p, idx) => {
            const isDefault    = p === 'Default';
            const hasLink      = !!profileSettings[p]?.linkedApp;
            const isGlobal     = !hasLink; // global profile = no linked app
            const isActiveGlob = isGlobal && p === activeGlobalProfile;
            const gearOpen     = settingsFor === p;
            const isRenaming   = renamingProfile === p;
            const isDragging   = dragIndex === idx;
            const isDropTarget = dragOverIndex === idx && dragIndex !== idx;
            // Default: only right-clickable when it's not already the active global (has "Set as Active" to show)
            // Others: always right-clickable when not in rename mode
            const canContextMenu = isDefault ? (isGlobal && !isActiveGlob) : !isRenaming;
            return (
              <div
                key={p}
                className={`profile-tab-group${isDragging ? ' dragging' : ''}${isDropTarget ? ' drop-target' : ''}`}
                draggable={!isDefault && !isRenaming}
                onDragStart={!isDefault ? e => handleDragStart(e, idx) : undefined}
                onDragOver={!isDefault ? e => handleDragOver(e, idx) : undefined}
                onDrop={!isDefault ? e => handleDrop(e, idx) : undefined}
                onDragEnd={!isDefault ? handleDragEnd : undefined}
                onContextMenu={canContextMenu ? e => handleContextMenu(e, p) : undefined}
              >
                {isRenaming ? (
                  <input
                    autoFocus
                  ...
                  <button
                    className={`profile-tab${p === activeProfile ? ' active' : ''}${hasLink ? ' linked' : ''}${isActiveGlob ? ' global-active' : ''}`}
